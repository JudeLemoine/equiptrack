...
function main() {
  const runSeed = db.transaction(() => {
    ...
    const insertUser = db.prepare("INSERT INTO User (id, name, email, role, department, phoneNumber, position, isAvatarIcon, avatarUrl) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)")
    ...
    for (let i = 0; i < adminProfiles.length; i++) {
      const id = generateId()
      insertUser.run(id, adminProfiles[i].name, `admin${i + 1}@equiptrack.local`, "ADMIN",
    ...
    for (let i = 0; i < fieldWorkerNames.length; i++) {
      const id = generateId()
      insertUser.run(id, fieldWorkerNames[i], `field${i + 1}@equiptrack.local`, "FIELD_WORKER",
    ...
    for (let i = 0; i < maintenanceNames.length; i++) {
      const id = generateId()
      insertUser.run(id, maintenanceNames[i], `maintenance${i + 1}@equiptrack.local`, "MAINTENANCE",
