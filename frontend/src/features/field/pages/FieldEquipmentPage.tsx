        ...
        <div className="space-y-2 md:col-span-3 lg:col-span-2">
          ...
          <div className="flex flex-wrap gap-1.5">
            {(['all', 'available', 'in_use', 'maintenance'] as const).map((val) => {
              const label = val === 'all' ? 'All statuses' : val === 'in_use' ? 'In use' : val.charAt(0).toUpperCase() + val.slice(1)
              return (
                <button
                  key={val}
                  onClick={() => setStatus(val as EquipmentStatus | 'all')}
                  className={`px-3 py-1 rounded-full text-sm font-medium transition-colors ${status === val ? 'bg-slate-900 text-white' : 'bg-white text-slate-600 border border-slate-200 hover:bg-slate-50 hover:border-slate-400'}`}
                >
                  {label}
                </button>
              )
